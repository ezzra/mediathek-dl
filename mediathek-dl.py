...
class MVW:
    ...
    def output_ssh(self, item):
        target_filepath, target_filename = self.get_target_data(item)
        ssh_cmd = ('ssh %s' % self.ssh) if self.ssh else ''
        os.system('%smkdir -p "\'%s\'"' % (ssh_cmd, target_filepath))
        os.system('%swget %s -O "\'%s\'"' % (ssh_cmd, item['link'], os.path.join(target_filepath, target_filename)))
    ...
    def output_curl(self, item):
        ...
        ssh_cmd = ('ssh %s' % self.ssh) if self.ssh else ''
        os.system('%smkdir -p "\'%s\'"' % (ssh_cmd, target_filepath))
        os.system('%scurl %s -# -o "\'%s\'"' % (ssh_cmd, item['link'], os.path.join(target_filepath, target_filename)))
        # hack to make it possible to exit with strg+c
        print()
        print('### sleeping for 5 seconds, use strg+c now if neccessary')
        time.sleep(5)
